...
def main():
    tasks = [
        ...
        },
        {
            ...
            "ay": [0.0, 20.0, 15.0, 40.0, 0.0],
            "obstacle": None
        }

    ]
    # Reference the task list above (place it before main or in global scope)
    for task in tasks:
        print(f"\n======== Running Task: {task['name']} ========")
        # Generate reference path using cubic spline interpolation
        ...
        while time < P.time_max:
            # Calculate the reference trajectory for MPC
            z_ref, target_ind = calc_ref_trajectory_in_T_step(node, ref_path, sp)
            z0 = [node.x, node.y, node.v, node.yaw]
            # No obstacle avoidance penalty in MPC, just follow the reference!
            ...
            x.append(node.x)
            y.append(node.y)
            yaw_list.append(node.yaw)
            v_list.append(node.v)
            t.append(time)
            d.append(delta_exc)
            a_list.append(a_exc)
            ...
            # Visualization (clear previous frame)
            plt.cla()

            # Draw obstacle if present
            if task.get("obstacle") is not None:
                obs = task["obstacle"]
            ...
            if x_opt is not None:
                plt.plot(x_opt, y_opt, color='darkviolet', marker='*', label='MPC Predicted Trajectory')
            plt.plot(cx, cy, color='gray', label='Reference Path')
            plt.plot(x, y, '-b', label='Actual Path')
            plt.plot(cx[target_ind], cy[target_ind], 'ro', label='Target Point')
            draw.draw_car(node.x, node.y, node.yaw, delta_exc, P)

            plt.axis("equal")
            plt.title(f"{task['name']} (MPC Path Tracking) v = {round(node.v*3.6, 2)} km/h")
            plt.xlabel("X [m]")
            plt.ylabel("Y [m]")
